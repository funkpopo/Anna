...
def test_flashqla_gated_delta_prefill_public_entry_uses_fast_recurrent_kernel() -> None:
    ...
    assert "return gated_delta_prefill_xpu(query_in, key_in, value_in, g_in, beta_in, state_in);" in body
    assert "flashqla_solve_wu_build_xpu" not in body
    assert "flashqla_chunk_gdr_fwd_xpu" not in body
...
@pytest.mark.skipif(not torch.xpu.is_available(), reason="XPU is required for the SYCL custom op test")
def test_flashqla_gdn_prefill_opt_in_short_prefill_uses_wrapper(
    ...
    layer = Qwen3GatedDeltaNet(config, 0).to("xpu", dtype=torch.float16)
    query = torch.randn(1, 63, 2, 8, device="xpu", dtype=torch.float16)
    key = torch.randn(1, 63, 2, 8, device="xpu", dtype=torch.float16)
    value = torch.randn(1, 63, 2, 8, device="xpu", dtype=torch.float16)
    g = torch.randn(1, 63, 2, device="xpu")
    beta = torch.sigmoid(torch.randn(1, 63, 2, device="xpu"))
