...
Anna 是一个本地推理运行时，面向大语言模型、多模态模型和语音模型。它提供 OpenAI 兼容 HTTP API，也提供命令行工具，用于本地生成、服务部署、性能测试、Qwen3-TTS 语音合成和 Qwen3-ASR 语音识别。

Anna 基于 PyTorch 构建，重点优化 Intel Arc / XPU。CPU 可用于开发、调试和小模型测试。

## 功能

- OpenAI 兼容接口：`/v1/chat/completions`、`/v1/completions`、`/v1/audio/speech`、`/v1/audio/transcriptions`、`/v1/models`
- 支持非流式和流式文本生成
- 支持 Chat、文本补全、多模态 Chat、函数调用和 reasoning 输出
- 支持 Qwen3-TTS 语音合成和 Qwen3-ASR 语音识别
...
| `qwen3_asr` | Qwen3-ASR | 语音识别 |
| `gemma4` | Gemma 4 | 文本、Chat、带音频的多模态 Chat |
| 其它值 | Qwen3.5 text / VL | 文本、Chat、图像/视频多模态 Chat |

文本生成模型使用 `anna-generate` 和 `anna-bench`。Qwen3-TTS 使用 `anna-speak`。Qwen3-ASR 使用 `anna-transcribe`。`anna-serve` 可加载任意支持的模型族；如果当前模型不支持某个接口，该接口会返回 API 错误。
...
Qwen3-ASR 运行时依赖 `qwen-asr` 和 `python-multipart`。它在 Anna 中按 Intel XPU 后端加载，不提供 CPU 或其它后端 fallback；如果 XPU 不可用，加载会直接失败。
...
Windows + Intel Arc A770 的本地环境示例：
...
如果只运行 Qwen3-ASR，不需要先构建 fused operator；它用于 Anna 的部分 XPU 自定义算子路径。
...
如果本地目录是 `models/Qwen3-ASR-0.6B`，把 `--model-dir` 改成对应目录即可。Anna 根据 `config.json` 中的 `model_type: qwen3_asr` 识别模型族，不依赖目录名。

也可以通过 OpenAI 兼容接口上传音频：
...
`language` 可省略，由 Qwen3-ASR 自动识别；如果确定语种，传入 `Chinese`、`English` 等模型支持的语言名称可以减少歧义。`response_format=text` 只返回纯文本，`response_format=verbose_json` 会返回模型 ID、语言和耗时；同时传 `return_timestamps=true` 时会请求时间戳输出。
...
| `/v1/audio/transcriptions` | `POST` | Qwen3-ASR 语音识别 |
