...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="auto",
        help="torch.compile mode for text generation. "
        "'auto' uses reduce-overhead on XPU and none elsewhere. "
        "First requests may pay a compile cost when not 'none'.",
    )
    parser.add_argument(
        "--compile-fullgraph",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=_positive_int,
        default=1,
        help="Maximum number of Qwen3-ASR audio chunks per XPU inference batch when serving ASR models.",
    )
    parser.add_argument(
        "--asr-max-new-tokens",
        type=_positive_int,
        default=8192,
        help="Maximum number of text tokens generated by Qwen3-ASR per chunk when serving ASR models.",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    engine = load_model_runtime_from_model_dir(
        ...
        asr_max_inference_batch_size=settings.asr_max_inference_batch_size,
